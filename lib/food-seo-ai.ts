...
- 소개글(intro)은 3문단 구조, 600~900자:
  1문단(200~250자): 가게명·지역·메뉴 핵심 소개. 제주/지역명/메뉴 키워드를 자연스럽게 녹여라.
  2문단(200~300자): 메뉴·식재료·조리 특징 + 제주 식문화 맥락. 검색자가 궁금해할 구체적 정보 포함.
  3문단(150~200자): 방문 팁 (웨이팅, 주차, 계절 추천, 제주 여행 동선 연계). 롱테일 키워드 자연스럽게 포함.
- FAQ는 방문객이 실제로 검색할 질문 5개 (위치/영업/가격/메뉴특징/주차 등). 답은 2~3문장, 데이터 근거 + 모르면 "방문 전 확인 권장".
- highlights는 짧은 특징 칩 3개 (검색 키워드가 될 만한 것으로).
- 과장·홍보문구 금지. 담백하고 정보성 있게. 사람과 검색엔진 모두 읽기 좋게.
- 반드시 유효한 JSON만 반환.`;
...
반환 형식 (JSON):
{
  "intro": "600~900자 SEO 소개글 (3문단, \\n\\n으로 문단 구분)",
  "faqs": [
    { "q": "질문 (검색 의도 기반)", "a": "2~3문장 답변" }
  ],
  "highlights": ["특징 키워드1", "특징 키워드2", "특징 키워드3"]
}

주의: intro는 반드시 3문단 구조로. 각 문단은 \\n\\n으로 구분. 합산 600자 이상이어야 한다.`;
