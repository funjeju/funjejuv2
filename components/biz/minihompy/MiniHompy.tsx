 ...
 * 메뉴 6종 전부 기능형: 다이어리(달력)·사진첩(갤러리)·방명록·스크랩(즐겨찾기)·방문자(접속로그)·BGM(유튜브).
 * 타이틀 클릭=홈(미니룸). 모든 값 실데이터.
 */
...
export function MiniHompy({ site, initialPosts }: { site: SiteSchema; initialPosts?: GuestPost[] }) {
  ...
  const [bubble, setBubble] = useState<{ text: string; leftPx: number } | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      fetch(`/api/biz/${site.slug}/visit`, { method: "POST", headers })
        .then((r) => r.json()).then((d) => { if (typeof d.today === "number") setCounts(d); }).catch(() => {});
    })();
  }, [site.slug, user]);
  ...
  const onRoomClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setHostFacing(pct > hostLeftPct ? "right" : "left"); setHostLeftPct(pct); triggerBob();
  }, [hostLeftPct, triggerBob]);
  const speak = useCallback(() => {
    const v = speakText.trim(); if (!v) return;
    const rr = roomRef.current?.getBoundingClientRect(); const width = rr?.width ?? 400;
    setBubble({ text: v, leftPx: Math.max(6, Math.min((hostLeftPct / 100) * width, width - 170)) });
    window.setTimeout(() => setBubble(null), 2600); setSpeakText(""); setSpeakOpen(false);
  }, [speakText, hostLeftPct]);

  const submitPost = useCallback(() => {
    const v = input.trim(); if (!v) return;
    const post = { name: "방문자", text: v };
    setPosts((p) => [post, ...p]); setInput("");
  ...
  const addDiary = async () => {
    const v = diaryText.trim(); if (!v) return;
    setDiaryText("");
    try {
      const r = await fetch(`/api/biz/${site.slug}/diary`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ date: selDate, text: v }) });
      const d = await r.json(); if (d.entry) setDiary((p) => [d.entry, ...p]);
    } catch { /* */ }
  };
  ...
  const diaryByDate = useMemo(() => {
    ...
    for (const e of diary) (map[e.date] ??= []).push(e);
    return map;
  }, [diary]);
  ...
  return (
    <div className="mh-page" style={{ background: room.pageBg, ["--accent" as string]: room.accent, ["--soft" as string]: room.accentSoft }}>
      <style>{`
        ...
        .mh-title{font-family:'Brush Script MT','Snell Roundhand',cursive;font-size:26px;color:var(--accent);line-height:1;cursor:pointer;}
        ...
        .mh-cal .d{aspect-ratio:1;display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:11px;border-radius:6px;cursor:pointer;border:1px solid transparent;}
      ...
      <div className="mh-wrap">
        ...
        <div className="mh-book">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 10 }}>
            <div>
              <div className="mh-title" onClick={() => setTab("home")} title="홈으로">{m.name} Minihome</div>
          ...
          {/* 홈 = 미니룸 */}
          {tab === "home" && (
            <div ref={roomRef} onClick={onRoomClick} style={{ position: "relative", height: 312, border: "1px solid #d8cba8", borderRadius: 8, overflow: "hidden", cursor: "pointer", background: room.bgImage ? `center/cover no-repeat url(${room.bgImage}), ${room.bg}` : room.bg }}>
              ...
              {bubble && <div style={{ position: "absolute", bottom: 84, left: bubble.leftPx, fontSize: 12, background: "#fffae0", border: "1px solid #e8d77a", borderRadius: 10, padding: "5px 9px", zIndex: 5, whiteSpace: "nowrap" }}>{bubble.text}</div>}
              <div style={{ position: "absolute", left: 8, bottom: 6, fontSize: 10, color: "#7a6a48", background: "rgba(255,255,255,.6)", borderRadius: 4, padding: "0 4px" }}>바닥 클릭→산책 · 말하기→말풍선</div>
            </div>
          )}
          ...
          {tab === "diary" && (
            <div>
              ...
              <div className="mh-cal" style={{ marginBottom: 4 }}>
                {["일", "월", "화", "수", "목", "금", "토"].map((w) => <div key={w} style={{ textAlign: "center", fontSize: 10, color: "#a89878" }}>{w}</div>)}
              </div>
              <div className="mh-cal">
                {Array.from({ length: firstDow }).map((_, i) => <div key={`b${i}`} />)}
                {Array.from({ length: daysIn }).map((_, i) => {
                  const ds = dateStr(calY, calM, i + 1);
                  const has = !!diaryByDate[ds];
                  const sel = ds === selDate;
                  return (
                    <div key={ds} className="d" onClick={() => setSelDate(ds)} style={{ background: sel ? "var(--accent)" : has ? "var(--soft)" : "#fff", color: sel ? "#fff" : "#5a4a32", border: "1px solid #eee5d4" }}>
                      {i + 1}{has && <span style={{ fontSize: 8, color: sel ? "#fff" : "var(--accent)" }}>●</span>}
                    </div>
                  );
                })}
              </div>
            ...
            <div>
              ...
              {gallery.length === 0 ? <p style={{ fontSize: 12, color: "#a89878", padding: "20px 0", textAlign: "center" }}>등록된 사진이 없어요.</p> : (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(120px,1fr))", gap: 8 }}>
                  {gallery.map((src, i) => <div key={i} style={{ aspectRatio: "1", borderRadius: 8, background: `center/cover no-repeat url(${src})`, border: "1px solid #e3d9c2" }} />)}
                </div>
              )}
            </div>
          )}
          ...
          {tab === "guestbook" && (
            <div>
              ...
              <div style={{ fontSize: 12, lineHeight: 1.7, minHeight: 80 }}>
          ...
          {tab === "visitor" && (
            <div>
              ...
              {visitors.length === 0 ? <p style={{ fontSize: 12, color: "#a89878", padding: "16px 0", textAlign: "center" }}>아직 다녀간 회원이 없어요.<br />(로그인 방문자만 기록돼요)</p> : (
                <div style={{ fontSize: 12 }}>
                  {visitors.map((v) => <div key={v.uid} className="mh-row"><span>🙋 {v.name}</span><span style={{ color: "#a89878" }}>{new Date(v.lastVisit).toLocaleDateString()}</span></div>)}
                </div>
              )}
            </div>
          )}
          ...
          {tab === "bgm" && (
            <div>
              ...
              {embed ? (
                <div style={{ position: "relative", paddingTop: "56.25%", borderRadius: 8, overflow: "hidden", marginBottom: 10 }}>
                  <iframe src={embed} title="BGM" allow="autoplay; encrypted-media" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", border: 0 }} />
                </div>
              ) : <p style={{ fontSize: 12, color: "#a89878", padding: "16px 0", textAlign: "center" }}>설정된 BGM이 없어요. 유튜브 링크로 배경음악을 넣어보세요.</p>}
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button className="mh-btn" onClick={saveBgm} style={{ background: "var(--accent)", color: "#fff", border: "1px solid var(--accent)" }}>{saveState === "saving" ? "저장…" : "설정"}</button>
              </div>
            </div>
          )}
        </div>
